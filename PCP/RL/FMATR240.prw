...
#IFDEF TOP
	...
	cGroupBy := "%"
	If ! lVEIC
		cGroupBy += " SB2.B2_COD, SB1.B1_TIPO, SB1.B1_GRUPO, SB1.B1_QB"
	Else	
		cGroupBy += " SB1.B1_CODITE, SB2.B2_COD, SB1.B1_TIPO, SB1.B1_GRUPO, SB1.B1_QB"
	EndIf
	...
	BeginSql Alias cAliasTOP
		...
		,SUM(SB2.B2_QTSEGUM) QTSEGUM, SUM(SB2.B2_QFIM2) QFIM2,	SB1.B1_QB QB
		%Exp:cSelect%
		...
		While !oReport:Cancel() .And. !(cAliasTOP)->(Eof())
			...
			If (mv_par14 == 1 .Or. ( mv_par14 <> 1 .and. QtdComp(nQuant) <> QtdComp(0))) .And. ;
				...
				oSection:Cell("B1_QB"):SetValue((cAliasTOP)->QB)
				...
				While !oReport:Cancel() .And. !SB2->(Eof()) .And. B2_COD == SB1->B1_COD
					...
					While !oReport:Cancel() .And. !SB2->(Eof())  .And. &(cCond2) == cQuebra2
						...
						If mv_par01 == 1 .Or. mv_par01 == 3
							...
							If mv_par19 == 1
								If lVersao
									oSection:Cell("NNR_DESCRI"):SetValue(NNR->NNR_DESCRI)
								Else
									oSection:Cell("B2_LOCALIZ"):SetValue(SB2->B2_LOCALIZ)
								EndIf
							EndIf	
...
cCabec1 := STR0025								//"CODIGO                         TP GRUP DESCRICAO                  UM FL"
cCabec2 := ''
If aTamFil[1] = 2
	If mv_par18 = 1
		cCabec1 += Space(1)+STR0028				//"AMZ  QTDE.1a.U.M.  UM   QTDE.2a.U.M."
	Else
		cCabec1	+= Space(1)+STR0026				//"AMZ   QUANTIDADE                  "
	EndIf
Else
	If mv_par18 = 1
		cCabec1 += Space(aTamFil[1]-2)+STR0028	//"AMZ  QTDE.1a.U.M.  UM   QTDE.2a.U.M."
	Else
		cCabec1	+= Space(aTamFil[1]-2)+STR0026	//"AMZ   QUANTIDADE                  "
	EndIf
EndIf
If mv_par19==1 .And. mv_par01==1
	cCabec1 += STR0021							//"    DESCRICAO ALMOX."
EndIf
...
#IFDEF TOP
	...
	cQuery += " GROUP BY"
	If ! lVEIC
		cQuery += " SB2.B2_COD, SB1.B1_TIPO, SB1.B1_GRUPO, SB1.B1_QB"
	Else	
		cQuery += " SB1.B1_CODITE, SB2.B2_COD, SB1.B1_TIPO, SB1.B1_GRUPO, SB1.B1_QB"
	EndIf
	...
	Do While !(cAliasTop)->(Eof())
		...
		If dbSeek(XSB1 + (cAliasTop)->COD)
			...
			If (mv_par14 == 1 .OR. ( mv_par14 <> 1 .and. alltrim(str(nQuant,16,2)) <> "0.00")) .And. ;
				...
				//codigo do produto
				If ! lVEIC
					@ Li, 00 pSay SubStr((cAliasTop)->COD, 1, 30)
				Else
					@ Li, 00 pSay (cAliasTop)->CODITE + " " + (cAliasTop)->COD
				EndIf	

				//tipo
				@ Li, 31/*16*/  + nCOL1 pSay (cAliasTop)->TIPO
				//grupo
				@ Li, 34/*19*/  + nCOL1 pSay (cAliasTop)->GRUPO
				...
				@ Li, 39/*24*/ + nCOL1 pSay If(Empty(cDesc),Left((cAliasTop)->DESCRI, 30),Left(cDesc,30))
				@ Li, 70/*66*/ + nCOL1 pSay (cAliasTop)->UM
				@ Li, 73/*69*/ + nCOL1 pSay AllTrim((cAliasTop)->FILIAL) 
				nColSV := nCOL1
				If aTamFil[1] > 2
					nCOL1  += aTamFil[1]-2
				Else
				...
				@ Li, 76 /*71*/ + nCOL1 pSay AllTrim((cAliasTop)->LOC)
				@ Li, 79 /*74*/ + nCOL1 pSay Transform( nQuant, cPict)//quantidade
				If mv_par18==1
					@ Li, 95 + nCOL1 pSay (cAliasTop)->SEGUM
					@ Li, 94 + nCOL1 pSay nQuant2 Picture cPict
					@ Li, 113 + nCOL1 pSay SubStr(If(SubStr((cAliasTop)->SITU,1,1)$'2', STR0019, STR0018), 1, 1)
					If mv_par19 == 1 .And. mv_par01==1
						@ Li, 115 + nCOL1 pSay Left((cAliasTop)->DESCARM,12)
					EndIf	
				Else
					@ Li, 113 + nCOL1 pSay SubStr(If(SubStr((cAliasTop)->SITU,1,1)$'2', STR0019, STR0018), 1, 1)
					If mv_par19 == 1 .And. mv_par01==1
		...
		If !(nTotQuebra==0) .And. If(aReturn[8]==4, !(cGrupoAnt==GRUPO) .Or. ((cAliasTop)->(EOF()) .And.Empty(cGrupoAnt)),If(aReturn[8]==2,!(cTipoAnt==TIPO),.F.)) .Or. ;
			...
			Else
				nCOL1 += 1
			EndIf
			@ Li, 33 + nCOL1 pSay If(Empty(cMens),SubStr(STR0016,1,5),STR0016 + cMens) //'Total do '
			@ Li, 73 /*63*/ + nCOL1 pSay Transform(nTotQuebra, cPict)/// valor total
			If mv_par18 == 1
				@ Li, 95 + nCOL1 pSay (cAliasTop)->SEGUM
				@ Li, 94 + nCOL1 pSay Transform(nTotQuebr2, cPict)
			EndIf
			Li += 2
			nCOL1 := nColSV
			nTotQuebra := 0
			...
			If nQtdProd > 1 .And. Alltrim(Str(aReturn[8])) $ "1|3" //-- So' totaliza Produto se houver mais de 1
				If (!(nTotProd==0).Or.!(nTotProdBl==0))
		        	...
		        	Else
						nCOL1 += 1
					EndIf				
					@ Li, 33 + nCOL1 pSay "(" + SubStr(STR0018,1,1)+ ") = " + OemToAnsi(STR0017) + OemToAnsi(STR0018) + Replicate('.',14)   //"Qtde. "###"Disponivel   "
					@ Li, 79 + nCOL1 pSay Transform((nTotProd-nTotProdBl), cPict)
					If mv_par18 == 1
						@ Li, 93 + nCOL1 pSay Transform((nTotProd2-nTotProdB2), cPict)
					EndIf
					Li++
					@ Li, 33 + nCOL1 pSay "(" + SubStr(STR0019,1,1)+ ") = " + OemToAnsi(STR0017) + OemToAnsi(STR0019) + Replicate('.',14)   //"Qtde. "###"Indisponivel "
					@ Li, 79 + nCOL1 pSay Transform(nTotProdBl, cPict)
					If mv_par18 == 1
						@ Li, 93 + nCOL1 pSay Transform(nTotProdB2,cPict)
					EndIf
					Li++
					If ! lVEIC
						@ Li, 33 + nCOL1 pSay OemToAnsi(STR0014) + Space(1) + AllTrim(Left(cCodAnt,15)) // 'Total do Produto'
					Else
						@ Li, 33 pSay OemToAnsi(STR0014) + Space(1) + (cCodite  + " " + cCodAnt) // 'Total do Produto'
					EndIf	
					@ Li, 79 + nCOL1 pSay Transform(nTotProd, cPict)
					If mv_par18 == 1
						@ Li, 93 + nCOL1 pSay Transform(nTotProd2,cPict)
					EndIf 
					nCOL1 := nColSV
					Li += 2
				EndIf
					...
					Do While lContinua .And. !SB2->(Eof()) .And. &(cCond2)
						...
						If mv_par01 == 1
							...
							If mv_par18 == 1
								...
								If mv_par19 == 1 .And. mv_par01==1
									If lVersao
										@ Li,105 + nCOL1 pSay NNR->NNR_DESCRI
									Else
										@ Li,105 + nCOL1 pSay SB2->B2_LOCALIZ
									EndIf
								EndIf
							Else
								...
								If mv_par19 == 1 .And. mv_par01==1
									If lVersao
										@ Li, 85 + nCOL1 pSay NNR->NNR_DESCRI
									Else
										@ Li, 85 + nCOL1 pSay SB2->B2_LOCALIZ
									EndIf
								EndIf
							EndIf
							nCOL1 := nColSV
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funcao    ³AjustaSX1 ³ Autor ³ Nereu Humberto Junior ³ Data ³22.11.2005³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Cria as perguntas necessarias para o programa               ³±±
±±³          ³e ajusta o SX1 para SIGAVEI, SIGAPEC e SIGAOFI  	          ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
PutSx1( "MTR240","19","Imprime descricao do Armazem ?","Imprime descripc. del almacen?","Print warehouse description ?","mv_chj",;
...
dbSeek(PADR(cPerg,nTamSX1)+'03') // Valid para evitar validação padrão do Fw, FWValidGC() 
