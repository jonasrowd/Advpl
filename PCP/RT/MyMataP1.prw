...
User Function MyMataP1
	...
	While l_Continua
		...
		While l_Produto == .F.
	   	...
	   	@ 1, 0 VTSAY "Arm. Origem: " VTGET c_Local Pict "@!" Valid f_VldArm(c_Local)
    	VTRead
		...
		If l_Rastro
			...
			c_Qry := " SELECT B8_LOTECTL, B8_DTVALID FROM " + RetSqlName("SB8") + " SB8 " + chr(13)
		  	c_Qry += " WHERE B8_FILIAL='" + XFILIAL("SB8") + "' AND B8_SALDO > 0 AND B8_DTVALID >= '" + DTOS(DDATABASE) + "' AND B8_LOCAL = '" + c_Local + "' AND B8_PRODUTO = '" + c_Produto + "' AND SB8.D_E_L_E_T_<>'*' " + chr(13)
...
/*	
		...
		While l_LocDest == .F.
			...
			dbSelectArea("NNR")
			dbSetOrder(1)
			If dbSeek(xFilial("NNR") + c_LocDest) .And. c_LocDest == "TE"
				l_LocDest := .T.
			Else
...
Static Function f_VldArm(c_Local)
	Local l_Ret := .F.
	...
	Else
		dbSelectArea("NNR")
		dbSetOrder(1)
		If dbSeek(xFilial("NNR") + c_Local) .AND. (c_Local == "P1" .OR. c_Local == "P4")
			l_Ret := .T.
		Else
			VTAlert("Armazém " + AllTrim(c_Local) + " inválido para essa operação. Por favor verifique se o código do armazém foi digitado corretamente.", "Aviso")
		Endif
	Endif
Return l_Ret