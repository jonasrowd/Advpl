...
User Function MyMata261
    Local c_Op       := Space(13)
    Local c_ProdOp   := Space(TamSX3("B1_COD")[1])
    Local c_LocalOp  := Space(TamSX3("B1_LOCPAD")[1])
    Local c_Produto  := Space(TamSX3("B1_COD")[1])
	Local c_Lote     := Space(TamSX3("D3_LOTECTL")[1] + 1)
	Local c_LocDest  := Space(TamSX3("B1_LOCPAD")[1])
    Local n_Quant    := 0
    Local n_ConfQtd  := 0
	...
	While l_Op == .F.
		...
		c_Qry := " SELECT C2_PRODUTO, C2_LOCAL, C2_DATRF, C2_STATUS FROM " + RetSqlName("SC2") + " SC2 WITH(NOLOCK) " + chr(13)
	  	c_Qry += " WHERE C2_FILIAL='" + XFILIAL("SC2") + "' AND C2_NUM+C2_ITEM+C2_SEQUEN = '" + c_Op + "' AND SC2.D_E_L_E_T_<>'*' " + chr(13)
		...
		dbGoTop()
		If QRY->(!EoF())
			If !Empty(QRY->C2_DATRF) .Or. QRY->C2_STATUS == "U"
				VTAlert("Ordem de Produção " + AllTrim(c_Op) + " está suspensa ou encerrada. Por favor verifique se o número da ordem de produção foi digitado corretamente.", "Aviso")
			Else
				c_ProdOp  := QRY->C2_PRODUTO
				c_LocalOp := QRY->C2_LOCAL
				l_Op      := .T.
			Endif
		Else
...
/*		dbSelectArea("SC2")
		...
		If Found()
			If !Empty(SC2->C2_DATRF) .Or. SC2->C2_STATUS == "U"
				VTAlert("Ordem de Produção " + AllTrim(c_Op) + " está suspensa ou encerrada. Por favor verifique se o número da ordem de produção foi digitado corretamente.", "Aviso")
			Else
				c_ProdOp  := SC2->C2_PRODUTO
				l_Op      := .T.
			Endif
		Else
			VTAlert("Ordem de Produção " + AllTrim(c_Op) + " não foi encontrada. Por favor verifique se o número da ordem de produção foi digitado corretamente.", "Aviso")
...
//   	SC2->(dbCloseArea())
   	...
   	dbSelectArea("SG1")
   	dbSetOrder(1)
   	dbSeek(xFilial("SG1") + c_ProdOp)
   	While SG1->(!EoF()) .And. SG1->(G1_FILIAL + G1_COD) == xFilial("SG1") + c_ProdOp
		AADD(a_Produtos, SG1->G1_COMP)
	...
	While l_Produto == .F.
		@ 1, 0 VTSAY "Produto: " VTGET c_Produto Pict "@!"
	    VTRead
		...
		If aScan(a_Produtos, c_Produto) == 0
			VTAlert("Produto " + AllTrim(c_Produto) + " não foi encontrado na estrutura do Produto " + AllTrim(c_ProdOp) + ". Por favor verifique se o código do produto foi digitado corretamente.", "Aviso")
		Else
			l_Produto := .T.
		Endif
   	End
	...
	c_Qry := " SELECT D4_QTDEORI FROM " + RetSqlName("SD4") + " SD4 WITH(NOLOCK) " + chr(13)
  	c_Qry += " WHERE D4_FILIAL='" + XFILIAL("SD4") + "' AND D4_COD = '" + c_Produto + "' AND D4_OP = '" + c_Op + "' AND SD4.D_E_L_E_T_<>'*' " + chr(13)
...
/*
   	dbSelectArea("SD4")
   	dbSetOrder(2)
   	dbSeek(xFilial("SD4") + c_Op + c_Produto)
   	While SD4->(!EoF()) .And. SD4->(D4_FILIAL + D4_OP + D4_COD) == xFilial("SD4") + c_Op + c_Produto
	...
	If l_Rastro
		...
		While l_Lote == .F.
			...
			c_Qry := " SELECT B8_LOTECTL, B8_DTVALID FROM " + RetSqlName("SB8") + " SB8 " + chr(13)
		  	c_Qry += " WHERE B8_FILIAL='" + XFILIAL("SB8") + "' AND B8_SALDO > 0 AND B8_DTVALID >= '" + DTOS(DDATABASE) + "' AND B8_LOCAL = 'E2' AND B8_PRODUTO = '" + c_Produto + "' AND SB8.D_E_L_E_T_<>'*' " + chr(13)
...
*/
	...
	n_QuantSZW := 0
   	...
   	dbSelectArea("SZW")
   	dbSetOrder(1)
   	dbSeek(xFilial("SZW") + c_Op + c_Produto + "E2")
   	While SZW->(!EoF()) .And. SZW->(ZW_FILIAL + ZW_OP + ZW_PRODUTO + ZW_LOCORIG) == xFilial("SZW") + c_Op + c_Produto + "E2"
	...
	While l_Quant == .F.
		...
		If SuperGetMV('MV_ESTNEG')=='N'
			...
			Else
				If n_Quant > 0 .And. n_Quant <= (n_QtdOri + n_QtdPerda - n_QuantSZW)
					l_Quant := .T.
				Else
	...
	While l_LocDest == .F.
		@ 5, 0 VTSAY "Local Destino: " VTGET c_LocDest Pict "@!"
	    VTRead
		...
		dbSelectArea("NNR")
		dbSetOrder(1)
		If dbSeek(xFilial("NNR") + c_LocDest) .And. c_LocDest $ "TI/TP" //.And. c_LocDest == c_LocalOp
			l_LocDest := .T.
		Else
			VTAlert("Armazem de destino inválido para esta operação ou inválido para esta ordem de produção. Por favor verificar se o código do armazém foi preenchido corretamente.", "Aviso")
		Endif
	End
    ...
    If Upper(c_Opcao) == "S"
		...
		Begin Transaction
			If c_LocDest == "TI"
				c_Doc := Getmv("BM_SEQTRTI")
			Elseif c_LocDest == "TP"
				c_Doc := Getmv("BM_SEQTRTP")
			Endif
...
//			MSExecAuto({|x,y| mata261(x,y)},aAuto,nOpcAuto)				
		...
		If dbSeek(xFilial("SD3") + c_Doc + c_Produto)
			While SD3->(!EoF()) .And. SD3->(D3_FILIAL + D3_DOC + D3_COD) == xFilial("SD3") + c_Doc + c_Produto
				If SD3->D3_EMISSAO == DDATABASE .AND. SD3->D3_LOCAL == 'E2' .AND. SD3->D3_TM == '999' .AND. SD3->D3_CF == 'RE4' .AND. SD3->D3_LOTECTL == c_Lote .AND. SD3->D3_CHAVE == 'E0' .AND. SD3->D3_QUANT == n_Quant
					dbSelectArea("SZW")
					dbSetOrder(4)
		...
		Else
			//Gravou a transferência com outro número de documento
		  	...
		  	c_Qry += " AND D3_COD = '" + c_Produto + "' AND D3_LOCAL = 'E2' " + chr(13)
		  	...
		  	c_Qry += " 	AND ZW_PRODUTO = '" + c_Produto + "' AND ZW_LOCORIG = 'E2' " + chr(13)
		...
		If l_Update
			...
			End Transaction
		Else
			VTAlert("Transferência do armazém E2 para o armazém " + c_LocDest + " foi realizada, porém houve falha no processo de gravação do registro na tabela SZW.", "Aviso")
			RestArea(a_Area)
		...
		If dbSeek(xFilial("SZW") + c_Doc + c_NumSeq)
			VTAlert("Transferência " + c_Doc + " do armazém E2 para o armazém " + c_LocDest + " realizada com sucesso.", "Aviso")
		Else
			VTAlert("Transferência " + c_Doc + " do armazém E2 para o armazém " + c_LocDest + " foi realizada, porém houve falha no processo de gravação do registro na tabela SZW.", "Aviso")
		Endif
