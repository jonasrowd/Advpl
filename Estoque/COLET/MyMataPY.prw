	...
	While l_Continua
		...
		While l_NumSeq == .F.
			...
			c_Qry := " SELECT D3_COD, D3_LOCAL, D3_LOTECTL, D3_OP, D3_QUANT FROM " + RetSqlName("SD3") + " SD3 WITH(NOLOCK) " + chr(13)
		  	...
		  	c_Qry += " AND D3_NUMSEQ IN (SELECT ZW_SEQORIG FROM " + RetSqlName("SZW") + " SZW WITH(NOLOCK) WHERE ZW_SEQORIG<>'' AND SZW.D_E_L_E_T_<>'*' AND ZW_LOCDEST = 'TR') "
