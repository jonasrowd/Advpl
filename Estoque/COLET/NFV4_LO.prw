	...
	While l_Continua
		...
		If l_Rastro
			...
			c_Qry := " SELECT B8_LOTECTL, B8_DTVALID FROM " + RetSqlName("SB8") + " SB8 " + chr(13)
		  	c_Qry += " WHERE B8_FILIAL='" + XFILIAL("SB8") + "' AND B8_SALDO > 0 AND B8_DTVALID >= '" + DTOS(DDATABASE) + "' AND B8_LOCAL = 'V4' AND B8_PRODUTO = '" + c_Produto + "' AND SB8.D_E_L_E_T_<>'*' " + chr(13)
    ...
    If Upper(c_Opcao) == "S"
		...
		If dbSeek(xFilial("SD3") + c_Doc + c_Produto)
			While SD3->(!EoF()) .And. SD3->(D3_FILIAL + D3_DOC + D3_COD) == xFilial("SD3") + c_Doc + c_Produto
				If SD3->D3_EMISSAO == DDATABASE .AND. SD3->D3_LOCAL == 'V4' .AND. SD3->D3_TM == '999' .AND. SD3->D3_CF == 'RE4' .AND. SD3->D3_LOTECTL == c_Lote .AND. SD3->D3_CHAVE == 'E0' .AND. SD3->D3_QUANT == n_Quant
					dbSelectArea("SZW")
					dbSetOrder(4)
		...
		Else
			//Gravou a transferência com outro número de documento
		  	...
		  	c_Qry += " AND D3_COD = '" + c_Produto + "' AND D3_LOCAL = 'V4' " + chr(13)
		  	...
		  	c_Qry += " 	AND ZW_PRODUTO = '" + c_Produto + "' AND ZW_LOCORIG = 'V4' " + chr(13)
		...
		If l_Update
			...
			End Transaction
		Else
			VTAlert("Transferência do armazém V4 para o armazém " + c_LocDest + " foi realizada, porém houve falha no processo de gravação do registro na tabela SZW.", "Aviso")
			RestArea(a_Area)
		...
		If dbSeek(xFilial("SZW") + c_Doc + c_NumSeq)
			VTAlert("Transferência " + c_Doc + " do armazém V4 para o armazém " + c_LocDest + " realizada com sucesso.", "Aviso")
		Else
