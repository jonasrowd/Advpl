...
*/
		...
		While l_LocDest == .F.
			...
			dbSelectArea("NNR")
			dbSetOrder(1)
			If dbSeek(xFilial("NNR") + c_LocDest) .And. c_LocDest $ "57"
				l_LocDest := .T.
			Else
...
//	@ 0, 0 VTSAY "Transferir (S/N)? " VTGET c_Opcao Pict "@!"
    ...
    If Upper(c_Opcao) == "S"
		...
		If l_Update
			...
			End Transaction
		Else
			VTAlert("Transferência do armazém LO para o armazém " + c_LocDest + " foi realizada, porém houve falha no processo de gravação do registro na tabela SZW.", "Aviso")
			RestArea(a_Area)
		...
		If dbSeek(xFilial("SZW") + c_Doc + c_NumSeq)
			VTAlert("Transferência " + c_Doc + " do armazém LO para o armazém " + c_LocDest + " realizada com sucesso.", "Aviso")
		Else
